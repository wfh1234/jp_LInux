...
d=`storcli /c2 show  |grep 'Product Name' |awk -F "=" '{print $2}' |awk '{print $NF}'`
...
raid_info=`paste id.txt r.txt`
				...
				e=`expr $c - 1`
				hba_level=`sas3ircu $e display |grep 'RAID level' |cut -f2 -d ':'`
				hba_status=`sas3ircu $e status |grep 'Volume state' |cut -f 2  -d :''`
				size=`sas3ircu $e display |grep 'Size (in MB)                            :' |cut -f 2 -d ":"`
if [ -z $size ]
then
echo "" | grep -v "^$"
else
...
cat .13.txt |grep -i "in use" |awk '{print $1"    "$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8}' |awk  -F "In" '{print $1}' >.14.txt
pci1=`lspci -v |grep -i sas |grep -B 1 $hba |grep -v "\--" |sed -n 3p |awk '{print $1}'`
